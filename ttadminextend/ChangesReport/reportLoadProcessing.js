$(function() {
    ...
    if (!jQuery.support.leadingWhitespace) {
        ...
        $('table tbody tr:nth-child(n+10)').css({
            display: 'none'
        }); /* hide 10+ rows */
    }
    ...
    setTimeout(function() {
        	...
        	//Add filter logic
         ﻿
        $('<div id="filterWrap">' + 	'<label for="inpCampaignIDNameFilter" class="filterLabel">Filter by Campaign ID/Name:</label><input type="text" id="inpCampaignIDNameFilter" name="inpCampaignIDNameFilter" placeholder="Type in part of a campaign name or id, filter would apply on every keystroke. Clear to see all." />\r\n ' + 	'<div></div>' + 	'<label for="inpCampaignLogFilter" class="filterLabel">Filter by Change Log keyword:</label><input type="text" id="inpCampaignLogFilter" name="inpCampaignLogFilter" placeholder="Type in part of a change log (this could be name, offer, etc), filter would apply on every keystroke. Clear to see all." />\r\n ' + 	'</div>').insertBefore('h1.campaignNameHead:eq(0)');

        	//Attach events
         ﻿
        $("#inpCampaignIDNameFilter").live("keyup", function(e) {	
            var inpVal = $(this).val(),
                	 selectHide = "h1.campaignNameHead:not(:contains('" + inpVal + "'))",
                selectShow = "h1.campaignNameHead:contains('" + inpVal + "')";	
            if (inpVal != '') {	 
                $(selectHide).hide();	 
        ...
        $("#inpCampaignLogFilter").live("keyup", function(e) {	
            var inpVal2 = $(this).val(),
                	 selectHide = "table tbody:not(:contains('" + inpVal2 + "'))",
                selectShow = "table tbody:contains('" + inpVal2 + "')";	
            if (inpVal2 != '') {	 
